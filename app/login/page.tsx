...
import { CurrentUserProfile } from "@/lib/auth/role-utils";
...
function LoginContent() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const destination = redirectPathFor(payload.user);
    ...
    <main className="flex min-h-screen items-center justify-center bg-gray-50 p-4 dark:bg-deep-navy-900">
      <Card className="w-full max-w-md overflow-hidden">
        <CardHeader className="border-b border-gray-200 dark:border-white/10">
          ...
          <CardTitle>TDS Management Login</CardTitle>
          <CardDescription>Use your approved admin or student account to continue.</CardDescription>
        </CardHeader>
        <CardContent className="pt-6">
          ...
          <p className="mt-5 text-center text-sm text-gray-500 dark:text-slate-400">
            Student account not approved yet?{" "}
